...
const CATEGORIES: TermCategory[] = ['structural', 'geotechnical', 'concrete', 'survey', 'general'];
...
export default function TermSearchPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((term) => {
      ...
      const matchesQuery =
        !q || term.term.toLowerCase().includes(q) || term.termBn.includes(query.trim()) || term.definition.toLowerCase().includes(q);
  ...
  return (
    <>
      ...
      <main className="container max-w-2xl py-10 md:py-14">
        ...
        <h1 className="mb-6 font-display text-2xl font-semibold tracking-tight md:text-3xl">{t.browseTerms}</h1>
        ...
        <div className="space-y-3">
          {filtered.map((term) => (
            <div key={term.id} id={term.id} className="rounded-lg border border-border p-4">
              <p className="font-display text-sm font-semibold">{locale === 'bn' ? term.termBn : term.term}</p>
              <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
                {locale === 'bn' ? term.definitionBn : term.definition}
              </p>
            </div>
          ))}
